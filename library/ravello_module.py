...
def create_app_from_template(client, module):
    ...
    for vm in read_app['vms']:
      ...
      try:
        updated_app = client.add_library_vm_to_application(appID, vm['baseVmId'])
      except Exception as e:
        log_contents = log_capture_string.getvalue()
        log_capture_string.close()
        module.fail_json(msg = '%s' % e,stdout='%s' % log_contents)
      ind = 0
      for v in updated_app['design']['vms']:
        vid = v['id']
...
def assert_vm_image_exists_in_ravello(client, module, baseVM):
    # Check VM exists in ravello
    images = client.get_images({'name': baseVM})
    if not images:
        module.fail_json(msg=\
                'FATAL ERROR nonexistent vm image called %s specified!' 
           % baseVM)
    return images[0]['id']
...
def assert_vm_valid(client, module, vm):
    ...
    set_default_if_missing(vm, 'baseVM', 'Empty')
    baseVmId = assert_vm_image_exists_in_ravello(client, module, vm['baseVM'])
    vm['baseVmId'] = baseVmId
    del vm['baseVM']
